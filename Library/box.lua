...
---An iterator direction: a string name, or the matching [`box.index`](lua://box.index) enum value.
---@alias box.iterator
---| "EQ" # comparing equal (the default)
---| "REQ" # comparing equal, in reverse order
---| "GT" # greater than
---| "GE" # greater than or equal
---| "LT" # less than
---| "LE" # less than or equal
---| "ALL" # all tuples
---| "BITS_ALL_NOT_SET" # all of the given bits are not set (BITSET)
---| "BITS_ALL_SET" # all of the given bits are set (BITSET)
---| "BITS_ANY_SET" # at least one of the given bits is set (BITSET)
---| "OVERLAPS" # the rectangle overlaps the given one (RTREE)
---| "NEIGHBOR" # tuples are sorted by distance to the given point (RTREE)
---| box.index.iterator
...
---@class box.sql.prepared
---@field stmt_id integer an identifier generated by a hash of the statement string
---@field params { name: string, type: string }[] parameter descriptions
---@field param_count integer number of parameters
---@field metadata? { name: string, type: string }[] present only for SELECT or PRAGMA statements and has the same contents as the result set metadata for `box.execute`
local prepared_table = {}

---Execute a statement that has been prepared with [`box.prepare()`](lua://box.prepare).
---
...
---That is, `prepared_table:execute()` and `box.execute(prepared_table.stmt_id)` do the same thing.
---
---@param extra_parameters? table an optional table to match placeholders or named parameters in the statement
---@return any query_result
function prepared_table:execute(extra_parameters) end
...
---That is, `prepared_table:unprepare()` and `box.unprepare(prepared_table.stmt_id)` do the same thing.
---
function prepared_table:unprepare() end

---Prepare the SQL statement contained in the `sql_statement` parameter.
---The syntax and requirements for `box.prepare` are the same as for [`box.execute()`](lua://box.execute).
---
---`box.prepare` compiles an SQL statement into byte code and saves the byte code in a cache.
---Since compiling takes a significant amount of time, preparing a statement will enhance
---performance if the statement is executed many times.
---
---@param sql_statement string statement, which should conform to [the rules for SQL grammar](doc://sql_statements_and_clauses)
---@return box.sql.prepared prepared_table with id and methods and metadata
function box.prepare(sql_statement) end

---Undo the result of an earlier [`box.prepare()`](lua://box.prepare) request.
---This is equivalent to standard-SQL `DEALLOCATE PREPARE`.
---
---@param prepared_statement box.sql.prepared | integer the result from `box.prepare()` or its `stmt_id`
function box.unprepare(prepared_statement) end
...
---It is similar to `msgpack.NULL`, `json.NULL` and `yaml.NULL`. So it is some not `nil` value, even if it is a pointer to NULL.
---
